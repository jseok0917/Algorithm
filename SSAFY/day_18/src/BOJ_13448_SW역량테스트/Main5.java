...
public class Main5 {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= N; i++) {
			M[i] = Long.parseLong(inputM[i-1]);
		}
		
		String[] inputP = br.readLine().split(" ");
		for (int i = 1; i <= N; i++) {
			P[i] = Long.parseLong(inputP[i-1]);
		}
		...
		for (int i = 1; i <= N; i++) {
			for (int j = 1; j <= T; j++) {
				...
				if (R[i] > j) {
					...
					int t = timeSum[i-1][(int) (j-R[i])];
					long temp = (long) (dp[i-1][(int) (j-R[i])]+M[i]-((long) (t+R[i])*P[i]));
					if (dp[i-1][j] < temp) {
						dp[i][j] = temp;
						timeSum[i][j] = (int)(timeSum[i-1][(int) (j-R[i])]+R[i]);
					} else {
						dp[i][j] = dp[i-1][j];
						timeSum[i][j] = timeSum[i-1][j];
					}
