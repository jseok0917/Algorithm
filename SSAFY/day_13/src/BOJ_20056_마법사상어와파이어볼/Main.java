...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		System.out.println(latticeInfo.toString());
		moveFireBall();
		System.out.println(latticeInfo.toString());
		afterMove();
		System.out.println(latticeInfo.toString());
			...
			for (int c = 0; c < N; c++) {
				...
				for (int j = 0; j < S; j++) {
					...
					case 0:
						...
						temp.get(lattice[(N+r+row[0]*s)%N][(N+c+col[0]*s)%N]).add(fireBall);
						
						break;
					case 1:
						temp.get(lattice[(N+r+row[1]*s)%N][(N+c+col[1]*s)%N]).add(fireBall);
						break;
					case 2:
						temp.get(lattice[(N+r+row[2]*s)%N][(N+c+col[2]*s)%N]).add(fireBall);
						break;
					case 3:
						temp.get(lattice[(N+r+row[3]*s)%N][(N+c+col[3]*s)%N]).add(fireBall);
						break;
					case 4:
						temp.get(lattice[(N+r+row[4]*s)%N][(N+c+col[4]*s)%N]).add(fireBall);
						break;
					case 5:
						temp.get(lattice[(N+r+row[5]*s)%N][(N+c+col[5]*s)%N]).add(fireBall);
						break;
					case 6:
						temp.get(lattice[(N+r+row[6]*s)%N][(N+c+col[6]*s)%N]).add(fireBall);
						break;
					case 7:
						temp.get(lattice[(N+r+row[7]*s)%N][(N+c+col[7]*s)%N]).add(fireBall);
						break;
