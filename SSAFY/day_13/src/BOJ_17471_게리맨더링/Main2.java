...
public class Main2 {
	...
	public static boolean isConnected(ArrayList<Integer> section) {
		if (!(section.isEmpty())) {
			...
			//connectedList가 두번쓰여 오류발생
			if (connectedList(section,root) != null) {
				System.out.println("isConnected내부 section"+section.toString());
				System.out.println("isConnected내부 connected"+connectedList(section, root).toString());
				return true;
//				return section.size() == connectedList(section, root).size();
	...
	public static void bruteForce(int N, int R) {
		...
		if (R == 0) {
			...
			System.out.println(complement(combination));
//			System.out.println(isConnected(combination));
//			System.out.println(isConnected(complement(combination)));
